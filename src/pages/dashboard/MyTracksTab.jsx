...
import { Search, Music, Pencil, Trash2, Eye, EyeOff, RefreshCw } from 'lucide-react'
import { useMyTracks } from '../../hooks/useDashboard'
    ...
    <div onClick={onCancel} style={{ position: 'fixed', inset: 0, zIndex: 100, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
      <div onClick={e => e.stopPropagation()} style={{ width: '100%', maxWidth: '320px', background: '#0d1a16', borderRadius: '16px', border: '1px solid rgba(248,113,113,0.3)', padding: '24px' }}>
        ...
        <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.5)', marginBottom: '20px', lineHeight: 1.5 }}>
          <span style={{ color: 'white', fontWeight: 600 }}>{track.title}</span>을 삭제하면<br />파일도 함께 삭제됩니다.
        </p>
    ...
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
          <span style={{ fontSize: '13px', fontWeight: 700, color: 'white', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '170px' }}>
            {track.title}
          </span>
          {!isPublic && (
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '2px', flexShrink: 0 }}>
        <button onClick={() => onTogglePublic(track)} title={isPublic ? '비공개' : '공개'}
...
export default function MyTracksTab({ userId }) {
  const [search,      setSearch]      = useState('')
  const [editTrack,   setEditTrack]   = useState(null)
  const [deleteTrack, setDeleteTrack] = useState(null)
  const [deleting,    setDeleting]    = useState(false)

  const { tracks, loading, error, reload, deleteTrack: doDelete, updateTrack } = useMyTracks(userId, search)
    ...
    <div style={{ padding: '12px 14px' }}>
      <div style={{ display: 'flex', gap: '10px', marginBottom: '14px' }}>
        <div style={{ flex: 1, position: 'relative' }}>
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="트랙 검색..."
            style={{ width: '100%', boxSizing: 'border-box', background: 'rgba(13,26,21,0.9)', border: '1.5px solid rgba(29,158,117,0.2)', borderRadius: '10px', padding: '10px 10px 10px 36px', color: 'white', fontSize: '14px', fontFamily: 'Inter, system-ui, sans-serif', outline: 'none' }} />
        </div>
      ...
      <p style={{ fontSize: '11px', color: 'rgba(255,255,255,0.3)', marginBottom: '8px' }}>
        {loading ? '로딩 중...' : `내 트랙 ${tracks.length}개`}
      </p>
        ...
        {error ? (
          ...
          [...Array(4)].map((_, i) => (
            <div key={i} style={{ display: 'flex', gap: '10px', padding: '10px 14px', borderBottom: '1px solid rgba(29,158,117,0.07)' }}>
              ...
              <div style={{ flex: 1 }}><div className="skeleton" style={{ height: '12px', width: '55%', marginBottom: '6px' }} /><div className="skeleton" style={{ height: '10px', width: '35%' }} /></div>
            </div>
          ))
        ) : tracks.length ? (
          tracks.map(t => <TrackRow key={t.id} track={t} onEdit={setEditTrack} onDelete={setDeleteTrack} onTogglePublic={handleTogglePublic} />)
        ) : (
          <p style={{ padding: '32px', textAlign: 'center', fontSize: '13px', color: 'rgba(255,255,255,0.3)' }}>업로드한 트랙이 없습니다</p>
        )}
      </div>
      ...
      {deleteTrack && (
        <DeleteConfirm track={deleteTrack} onConfirm={handleDelete} onCancel={() => setDeleteTrack(null)} loading={deleting} />
      )}
    </div>
  )
}
